...
namespace Finite_State_Machine_Designer.Components.Pages
{
    ...
    {
        private async ValueTask GetFullFsm(FiniteStateMachine fsm, CancellationToken cancelToken)
        {
            if (cancelToken.IsCancellationRequested)
                return;
            await using DbContext dbContext = await DbFactory.CreateDbContextAsync(cancelToken);
            EntityEntry<FiniteStateMachine> fsmEntry = dbContext.Attach(fsm);
            await fsmEntry.Collection(stateMachine => stateMachine.States).LoadAsync(cancelToken);
            await fsmEntry.Collection(stateMachine => stateMachine.Transitions).LoadAsync(cancelToken);
